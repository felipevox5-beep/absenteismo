...
// Suporte global para serialização de BigInt (necessário para IDs de Absence e AuditLog)
...
async function logAudit(userId: number | null, action: string, tableName?: string, recordId?: number, oldValues?: any, newValues?: any, req?: any) {
  try {
    // Only link to SystemUser if it's a valid positive ID (SystemUsers start at 1)
    const validUserId = (userId && userId > 0) ? userId : null;
...
// =====================================================
// EMERGENCY AUTO-MIGRATION (POSTGRESQL ONLY)
// =====================================================
async function runEmergencyMigrations() {
  console.log('🔧 Iniciando auto-migração de emergência...');
  try {
    ...
    // 2. Corrigir tabela absences - Adicionar TODAS as colunas que podem estar faltando
    await prisma.$executeRawUnsafe(`ALTER TABLE "absences" ADD COLUMN IF NOT EXISTS "time" VARCHAR(10);`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "absences" ADD COLUMN IF NOT EXISTS "status" VARCHAR(50) DEFAULT 'registrado';`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "absences" ADD COLUMN IF NOT EXISTS "end_date" DATE;`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "absences" ADD COLUMN IF NOT EXISTS "duration_days" INTEGER;`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "absences" ADD COLUMN IF NOT EXISTS "cid_id" INTEGER;`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "absences" ADD COLUMN IF NOT EXISTS "created_by" INTEGER;`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "absences" ADD COLUMN IF NOT EXISTS "atestado_file_path" VARCHAR(500);`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "absences" ADD COLUMN IF NOT EXISTS "atestado_data" BYTEA;`);
    ...
    await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "absences_employee_id_idx" ON "absences"("employee_id");`);
    await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "absences_cid_idx" ON "absences"("cid");`);

    console.log('✅ Auto-migração concluída com sucesso!');
  } catch (error) {
    console.error('⚠️ Falha na auto-migração:', error);
  }
}

async function startServer() {
  // Rodar migrações de emergência antes de iniciar o servidor
